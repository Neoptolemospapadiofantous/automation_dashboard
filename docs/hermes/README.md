...
Local CI + audit tooling for automation_dashboard. The LLM/fleet layer is invoked manually (composer scripts or interactive Claude Code slash commands); the no-LLM collectors (`audit_sentinel`, `update_inspector`, `system_check`) also run on a prod schedule via `routes/console.php` and write their findings to `data/agents/*/`. This repo only *produces* those findings; notification/delivery lives in the separate hermes-slack project.
...
## Alerting (CRITICAL/FAIL → notification)

The collectors are scheduled in prod but only *write report files* to
`data/agents/*/` — nothing reads them unless someone runs `composer hermes-status`.
This repo is purely the *producer* of those findings; turning an actionable finding
into a notification (the previous `hermes:alert`/`slack:digest` lanes) lives in the
separate **hermes-slack** project, which is the sole consumer/notifier.
