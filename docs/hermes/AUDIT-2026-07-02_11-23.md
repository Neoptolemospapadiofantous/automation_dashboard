...
### Medium
- **dns-rebinding-toctou (accepted risk, tracking only)** — `app/Runtime/Automation/OutboundGuard.php:20-24`. The guard resolves DNS at check time but Guzzle re-resolves at request time; the comment explicitly acknowledges the rebinding window. Proper closure means pinning the request to the vetted IP (curl `CURLOPT_RESOLVE`). Low practical urgency given operator-only URL authoring — record as a known limitation rather than fix now.
...
2. When touching `OutboundGuard` next, consider IP-pinning via curl options to close the DNS-rebinding window for good.
