...
3. ~~Extend `scripts/agents/audit_sentinel.sh` with a check for `Http::` call sites lacking both `->timeout(` and redirect hardening.~~ — DONE same session: check 8 flags `app/` files using `Http::` without a `timeout(` call (MEDIUM) and trips HIGH if `AutomationCaller` ever loses `'allow_redirects' => false` or the `CURLOPT_RESOLVE =>` pin. All three verified to fire by stripping the hardening from a scratch copy.
